...
#include <memory>
#include <cmath>
#include "test6005.hpp"
#include "utils.hpp"
#include "CL/cl.hpp"
std::vector<int> test6005(std::vector<int> xs, int add) {
...
int sum(int);
int lfHrUSr6I0rGo(int);

__constant int xs[{!xs_length}]  = {!xs_contents};
__constant int add = {!add_contents};
int sum(int v1) {
    return lfHrUSr6I0rGo(v1);
};
int lfHrUSr6I0rGo(int x) {
    return ((x) + (add)) + (5);
}
kernel void GPUMonad(__global int *out) {
out[get_global_id(0)] = sum(xs[get_global_id(0)]);
}
...
kernelSource = replace(kernelSource, "{!add_contents}", std::to_string(add));
