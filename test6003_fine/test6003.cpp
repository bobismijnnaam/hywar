...
out[atomic_inc(count)] = ps[get_global_id(0)];
    ...
    auto h_count = std::unique_ptr<int[]>(new int[n]);// std::make_unique<int[]>(1);
    h_count[0] = 0;
    ...
    try {
        ...
        std::cout << "h_count: " << h_count[0] << "\n";
        std::cout << "Bwaaaaaaaah";
        ...
        cl::NDRange globalSize((int) (std::ceil(n / (float)workGroupSize) * workGroupSize));
